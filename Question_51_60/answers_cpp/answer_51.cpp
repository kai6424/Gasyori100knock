...
// Morphology Gradient
cv::Mat Morphology_Gradient(cv::Mat eroded, cv::Mat delated)
{
    int height = eroded.cols;
    int width = eroded.rows;

    // output image
    cv::Mat out = eroded.clone();
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            out.at<uchar>(y, x) = (uchar)std::abs((int)out.at<uchar>(y, x) - (int)delated.at<uchar>(y, x));
        }
    }
...
int main(int argc, const char *argv[])
{
    ...
    // erode image
    cv::Mat eroded = Morphology_Erode(otsu, 1);

    // delate image
    cv::Mat delated = Morphology_Dilate(otsu, 1);

    // morphology gradient
    cv::Mat out = Morphology_Gradient(eroded, delated);

    // save result
    cv::imwrite("answer_51.jpg", out);
